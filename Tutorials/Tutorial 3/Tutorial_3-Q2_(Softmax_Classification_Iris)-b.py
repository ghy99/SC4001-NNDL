...
def plot_err_entropy(train_acc, train_loss, test_acc, test_loss, no_epochs, batch_size):
    plt.clf()
    plt.figure(1)
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.savefig(f'./graphs/Tutorial-3/loss-batch-size-{batch_size}.png')
    # plt.show()
    ...
    plt.savefig(f'./graphs/Tutorial-3/accuracy-batch-size-{batch_size}.png')
    # plt.show()
...
def plot_losses(losses, no_epoch,batch_sizes):
    plt.clf()
    fig = plt.figure(1)
    ax = plt.subplot(111)
    ...
    for i in range(len(losses)):
        line, = ax.plot(range(no_epoch), losses[batch_sizes[i]], label=f"Batch {batch_sizes[i]}")
    ...
    plt.ylabel("Loss")
    plt.savefig(f'./graphs/Tutorial-3/loss-comparison-between batches.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    for batch_size in batch_sizes:
        ...
        plot_err_entropy(train_acc_, train_loss_, test_acc_, test_loss_, no_epochs, batch_size)
        w = model.state_dict()["softmax_layer.0.weight"].numpy()
        b = model.state_dict()["softmax_layer.0.bias"].numpy()

        print(f"\n\nConverged weight:\n{w}\nConverged bias:\n{b}")
        print()
        track_train_loss[batch_size] = (train_loss_)
        track_test_loss[batch_size] = (test_loss_)
        end = time.time()
        time_taken.append(start - end)
    
    plot_losses(track_train_loss, no_epochs, batch_sizes)